...
const ToolbarModalItem = ({ type, identifier, item, switchModal}) => {
    const location = useLocation()
    const history = useHistory()
    const selected = getValue(location.search, type)
    const value = item[identifier]
    const visible = value === selected ? true : false
    const onClick = () => {
        let queryString = location.search
        if(!visible) {
            let search = updateValue(queryString, type, value) 
            history.push({
                pathname: pathUri.issue,
                search: search,
            })
        } else {
            let search = removeQuery(queryString, type, value)
            history.push({
                pathname: pathUri.issue,
                search: search,
            })
